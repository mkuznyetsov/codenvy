...
public class TestNodeConfigHelperCodenvy3 {
    ...
    public void testRecognizeNodeTypeBy() {
        Config testConfig = new Config(ImmutableMap.of(Config.ADDITIONAL_RUNNERS, TEST_NODE_DNS));
        NodeConfigHelperCodenvy3Impl testConfigUtil = new NodeConfigHelperCodenvy3Impl(testConfig);

        assertEquals(testConfigUtil.recognizeNodeTypeFromConfigBy(TEST_NODE_DNS), NodeConfig.NodeType.RUNNER);
    }

    @Test
    public void testGetPropertyNameBy() {
        assertEquals(spyConfigHelper.getPropertyNameBy(NodeConfig.NodeType.RUNNER),
                     ADDITIONAL_RUNNERS_PROPERTY_NAME);
        assertEquals(spyConfigHelper.getPropertyNameBy(NodeConfig.NodeType.BUILDER),
                     "additional_builders");
    }
